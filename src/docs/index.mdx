...
**注意** :dog:，因为需要一些MDX插件来正确解析MD语法，但是docz的[bug未修复](https://github.com/doczjs/docz/issues/1629)，所以要解析正确的格式，只能手动将项目根目录下的`gatsby-config.fix.js`内容替换`node_modules/gatsby-theme-docz/gatsby-config.js`（不替换也不影响本地运行，只是有些MD语法扩展解析不了）。
